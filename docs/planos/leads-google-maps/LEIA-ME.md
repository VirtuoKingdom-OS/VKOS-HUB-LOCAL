...
Comparamos quatro caminhos (registrado em `docs/decisoes/2026-07-18-leads-google-maps.md`): API oficial do Google Places (sem campo de email, e os próprios Termos Adicionais do Google proíbem usar os dados pra montar lista de email ou telemarketing, mesmo vindo da API oficial e mesmo com crédito Google Cloud sobrando), scraping direto via Playwright (descartado, risco de bloqueio e contradiz "geração verificada, não confiada"), serviço de terceiro (mais rico, risco contratual fica com o provedor), e um híbrido de API oficial mais enriquecimento por sessão de IA (mais lento, gasta crédito de sessão).
...
> Execute o plano da pasta docs/planos/leads-google-maps

...
`docs/decisoes/2026-07-27-o-glow-so-marca-estado.md`,
`docs/decisoes/2026-07-27-a-fonte-embarcada.md` e
`docs/decisoes/2026-07-27-duas-linhas-e-a-escada-de-superficie.md`.
...
Cinco degraus. Antes eram 68 declarações com 26 valores distintos, o único eixo do sistema sem
escala. Fechada em 2026-07-27, ver `docs/decisoes/2026-07-27-a-escala-de-opacidade.md`.

| Token | Valor | Onde se usa |
|---|---|---|
| `--op-plena` | 1 | estado normal |
| `--op-secundaria` | 0.72 | elemento de apoio |
| `--op-fraca` | 0.5 | recuado, fio de separador |
| `--op-apagada` | 0.32 | desabilitado, decoração |
| `--op-fantasma` | 0.16 | fora de foco em canvas denso (só o Mapa usa) |

Duas regras, as duas com trava em `escalas.test.ts`:

**Nenhum valor fora da escala.** `0` e `1` seguem livres, e dentro de `@keyframes` a opacidade é
trajetória de animação, não hierarquia, então fica de fora.

**Opacidade não encosta em texto.** Nenhum degrau é seguro sobre texto, e a trava de contraste não
percebe porque mede token contra token, não a composição com o fundo. Medido no Claro, com
`--texto-fraco` #5d6d66 sobre branco:
...
Rótulo que precisa ficar discreto usa peso, tracking ou token de cor mais fraco. A trava isenta
seletor de ícone e `svg`, porque SVG pinta o traço com `color` via `currentColor` e cairia no
mesmo padrão sem ser texto.

### 3.8 O rótulo de grupo

O quarto degrau de texto, `--texto-rotulo`, existe só para rótulo de seção em caixa alta (CORE,
GESTÃO, Conteúdo, Fontes de dados). Ver
`docs/decisoes/2026-07-27-o-rotulo-de-grupo-recua.md`.

| canal | rótulo de grupo | item de menu |
|---|---|---|
| tamanho | 11px, o piso | 14px |
| peso | 400 | 600 |
| contraste | 4,69:1 | 7,87:1 |
| tracking | 0,14em | normal |

Dois pontos que não são óbvios:

**O tracking abre, não fecha.** Em caixa alta, juntar as letras adensa a mancha e a palavra vira
um bloco sólido que compete com o item de menu. Espalhado, o mesmo texto dilui e recua. Uma
tentativa de fechar para 0,06em piorou o problema que tentava resolver.

**O token é calibrado pelo pior fundo, não pelo mais comum.** `.rotulo-secao` veste a barra
lateral, que é cartão, mas também o painel de peças, que se apoia no canvas. Calibrar pelo cartão
deixaria o painel em 4,18:1. Os dois pares estão no `contraste.test.ts`.
...
Ver `docs/decisoes/2026-07-27-ordem-da-cascata-com-layer.md`.
...
Ver `docs/decisoes/2026-07-27-a-fonte-embarcada.md`.

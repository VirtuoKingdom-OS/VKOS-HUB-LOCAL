# VKOS 2: VirtuoKingdom Operational System
...
O VKOS 2 não é só marketing: é também o sistema operacional de construção do dono. Ele pode
...
- Quando o projeto tem interface, a camada visual inteira se aplica: cartela, princípios do
  formato mais próximo e teste final ("Antes de qualquer peça visual", acima).
...
- `marca/`: estratégia. `posicionamento.md` (do `/posicionamento`) e `ikigai.md` (do `/ikigai`).
...
- `conteudo/`: tudo que você gera vai aqui, em subpastas `AAAA-MM-DD-tema-curto/`. Projetos
  livres também, salvo pasta própria combinada no `/projeto`.

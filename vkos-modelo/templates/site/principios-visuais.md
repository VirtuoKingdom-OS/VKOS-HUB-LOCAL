...
- [ ] Todo botão e cartão clicável é um `<a>`.
...
3. **Todo botão ou cartão clicável é um `<a>`**, mesmo desativado ou "em breve": nesse caso um `<a>` SEM atributo href e com `aria-disabled="true"`, que não navega. Nunca use pointer-events pra desligar interação.

...
Toda sessao aqui le alguns arquivos antes de agir, definido no `CLAUDE.md`: `docs/contexto/visao.md`,
`docs/contexto/arquitetura.md`, `docs/contexto/roadmap.md` (e `docs/contexto/ecossistema.md` quando a tarefa toca o
produto), a pasta `docs/decisoes/`, o proprio `CLAUDE.md`, o `MEMORY.md` da memoria e o
...
1. **Fica desatualizado.** Uma fase do roadmap fechou mas continua marcada como pendente, a
   ...
   chat com o Jesse e nunca virou arquivo em `docs/decisoes/`. Ai a proxima sessao trabalha em cima de
...
Leia `docs/contexto/roadmap.md`, `docs/contexto/arquitetura.md` e `docs/contexto/visao.md`. Se a pasta `docs/contexto/`
...
- **Os arquivos de contexto** (os que a sessao le toda vez). Passe o olho no tamanho e no estado de
  `docs/contexto/visao.md`, `docs/contexto/arquitetura.md`, `docs/contexto/roadmap.md`, `docs/contexto/ecossistema.md`,
...
- **As decisoes.** Liste `docs/decisoes/` pelas datas. Veja se as ultimas decisoes ja estao refletidas no
...
- **Decisoes vs. contexto.** Foi tomada uma decisao com o Jesse (no chat ou numa rodada) que ainda
  nao virou arquivo em `docs/decisoes/`, ou virou arquivo mas o roadmap e a arquitetura nao absorveram?
...
- **Informacao repetida.** O mesmo fato em dois arquivos de contexto (ex: a mesma decisao contada por
  extenso no roadmap **e** na arquitetura **e** no arquivo de `docs/decisoes/`). Repetido, pode divergir e
  dobra o que a sessao le. Escolha uma **fonte da verdade** (o arquivo de `docs/decisoes/` costuma ser a
...
1. [atualizar] docs/contexto/roadmap.md (Fase 1): ja fechou no codigo, o terminal foi removido de app/
   e o menu so tem Carrossel e Site. A linha ainda diz "em execucao".
2. [atualizar] docs/contexto/arquitetura.md: o server ganhou app/server/src/meta/ que o documento nao
   descreve (rodada 12).
3. [enxugar] app/CONTRATO.md: ainda cita o backend PTY do terminal, que nao existe mais (referencia
   morta).
4. [enxugar] docs/contexto/roadmap.md + docs/decisoes/2026-07-14-tres-temas.md: a decisao dos 3 temas esta
...
```
docs/contexto/roadmap.md, Fase 1
...
4. **Uma fonte da verdade por informacao.** Decisao que vive por extenso em tres arquivos vira
   divergencia. A decisao mora em `docs/decisoes/`, o roadmap e a arquitetura so apontam.

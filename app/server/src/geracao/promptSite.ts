// O PROMPT DO SITE GUIADO.
//
// Ele morava em `web/src/componentes/criacao/promptSite.ts` e subiu pro
// servidor em 2026-08-04, pelo mesmo motivo do promptCarrossel.ts: a fila do
// Assistente monta prompt sem navegador aberto, e a versao que o servidor tinha
// era uma reescrita de dezoito linhas que perdia quase tudo daqui.
//
// O arquivo do web reexporta daqui, e as fixtures de la provam que a mudanca de
// casa nao mudou uma virgula do texto.

import type { DadosSiteEntrada as DadosSite } from "./modelo.js";
...
function formatoLinha(formato: DadosSite["formato"]): string {
  if (formato === "completo") {
...
function ctaLinhas(objetivo: DadosSite["objetivo"], link: string): string {
...
function imagensLinhas(dados: DadosSite, pasta: string): string {
...
// da escala do estilo.
function visualLinha(dados: DadosSite): string {
  return [
...
function marcadoresLinhas(formato: DadosSite["formato"]): string {
...
export function montarPromptSite(dados: DadosSite, pasta: string): string {

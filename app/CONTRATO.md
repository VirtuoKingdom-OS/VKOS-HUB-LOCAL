...
Identidade visual do app: identidade v3 desde 2026-08-05 (ver `docs/decisoes/2026-08-05-identidade-v3.md`). O Escuro quase preto e quente é o padrão; o Claro usa creme de laboratório. Geist é embarcada, títulos usam voz editorial de peso 400, menta sinaliza marca e estado vivo, e carvão e textura têm superfícies contratadas. O contrato completo mora em `docs/contexto/identidade-visual.md`; a implementação vem de `web/src/estilos/global.css` (base e escalas), `primitivas.css` (a camada de componentes) e `visual-hub.css` (o valor final de cada token por tema). Toda cor passa por token e as travas em `web/src/estilos/*.test.ts` são executáveis. UI inteira em português brasileiro.

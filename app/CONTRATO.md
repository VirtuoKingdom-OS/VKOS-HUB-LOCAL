...
- Quando uma sessão com `skill` conferível e `pastaAlvo` conclui bem, o gerenciador roda a auditoria completa do deploy (`publicacao/auditoria.ts`: estrutural + visual no navegador) na peça. Só dispara se a `pastaAlvo` existe como peça de site (tem `index.html` na raiz e não é carrossel).

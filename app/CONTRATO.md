...
- Quando uma sessão com `skill: "site"` e `pastaAlvo` conclui bem, o gerenciador roda a auditoria completa do deploy (`publicacao/auditoria.ts`: estrutural + visual no navegador) na peça. Só dispara se a `pastaAlvo` existe como peça de site (tem `index.html` na raiz e não é carrossel).
...
- Catálogo fixo em `server/src/conexoes/catalogo.ts`: GitHub, Netlify, Notion e Google Calendar. Vercel, Meta e Google Ads não aparecem nesta versão. Cada entrada descreve campos, transporte e montagem do servidor MCP quando aplicável.
...
- A auditoria continua intacta como barreira de deploy. GitHub e Netlify permanecem bloqueados enquanto qualquer erro existir.

## Publicação de sites

- `server/src/publicacao/` é o módulo determinístico de publicação. Estado em `app/dados/workspaces/<id>/publicacoes.json`, com registro separado por pasta de peça e destino.
- `GET /api/publicacao/:pasta` responde `{ github: { conectado }, netlify: { conectado }, registro, auditoria }`. A auditoria combina a verificação estrutural com a inspeção real de todas as páginas em 390 px e 1440 px, rolagem completa, modo sem JavaScript e `prefers-reduced-motion`. O resultado fica em cache por até 2 minutos, vinculado à versão exata dos arquivos da peça, para manter o painel responsivo sem reutilizar resultado obsoleto.
- `POST /api/publicacao/:pasta/github` cria ou reutiliza um repositório privado, envia blobs, cria uma árvore completa sem `base_tree`, cria o commit e atualiza a referência. Responde `{ repo, url, branch, em }`.
- `POST /api/publicacao/:pasta/netlify` cria ou reutiliza o site, envia um ZIP completo e aguarda o deploy por até 90 segundos. Responde `{ siteId, url, em, pendente? }`.
- Os dois POST repetem a mesma auditoria antes de qualquer chamada externa. Ela bloqueia CSS ou recurso quebrado, erro de página, rolagem horizontal, conteúdo oculto sem JavaScript ou após a rolagem, movimento incompatível com redução de movimento, wrappers genéricos diretos no `body`, destinos desabilitados inválidos, controle de menu sem `button`, contraste insuficiente e os padrões visuais proibidos pelo contrato.
...
- O HTML da peça é a fonte da verdade local; nada do fluxo local muda. A conversão Astro acontece só na publicação, em `.astro-build/` dentro da peça, pasta interna adicionada a `PASTAS_INTERNAS` (auditoria, previews, watch e coleta ignoram).
...
- `resolverModoPublicacao`: `"astro"` só com auditoria estrutural válida, multipágina, marcadores em todas as páginas e nav/rodapé realmente compartilháveis; senão `"html"`. A previsão usa o mesmo contrato do conversor, portanto não anuncia Astro quando a identidade do layout já exigiria fallback. Scripts diretos do body são preservados na página Astro correspondente e podem variar entre páginas. `GET /api/publicacao/:pasta` ganhou `modoPrevisto`. Os POSTs ganharam `modo` e `avisos: string[]`; a barreira de auditoria roda antes, intocada. Modo astro: GitHub recebe a árvore fonte do `.astro-build/` (sem node_modules nem dist), Netlify recebe o ZIP do dist. Qualquer falha de conversão, motor ou build cai pro modo html na mesma requisição com aviso. `publicacoes.json` guarda o modo por destino.
- `POST /api/publicacao/:pasta/ensaiar-astro` (interna, diagnóstico): converte e builda sem publicar, responde `{ ok, modo, paginas, avisos }`.
- TelaSite: badge "Publica como projeto Astro" quando `modoPrevisto === "astro"`, avisos dos POSTs no padrão existente. Limitação conhecida: o layout compartilhado publicado perde o `aria-current` de link ativo.
